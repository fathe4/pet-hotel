...
const MyHotels = () => {
  ...
  const { data, isLoading: IsListingLoading } = useGetListingsQuery({
    ...query,
  });
  const hostels: any = data?.hostels;

  const [deleteListing] = useDeleteListingMutation();
  const usersWithKeys = hostels?.map((user: SafeListing) => ({
    ...user,
    key: String(user.id),
  }));
  ...
  const handleDelete = async (id: string) => {
    const result: any = await deleteListing(id);
    if (result?.data) {
